---@class geekvim.util.cmp
local M = {}

---@alias geekvim.util.cmp.Action fun():boolean?
---@type table<string, geekvim.util.cmp.Action>
M.actions = {
  -- Native Snippets
...
function M.expand(snippet)
  ...
  -- See: https://github.com/GeekVim/GeekVim/issues/3199
...
function M.setup(opts)
  ...
  cmp.event:on("confirm_done", function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
      GeekVim.cmp.auto_brackets(event.entry)
    end
  end)
  cmp.event:on("menu_opened", function(event)
    GeekVim.cmp.add_missing_snippet_docs(event.window)
  end)
end
