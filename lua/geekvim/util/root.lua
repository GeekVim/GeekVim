...
function M.detectors.lsp(buf)
  ...
  local clients = GeekVim.lsp.get_clients({ bufnr = buf })
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  GeekVim.info(lines, { title = "GeekVim Roots" })
...
function M.setup()
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("geekvim_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return GeekVim.is_win() and ret:gsub("/", "\\") or ret
end
